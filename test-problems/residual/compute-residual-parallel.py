...
import meshio, re, argparse, os, sys, time, subprocess
...
def read_transient_data(folder, id):
    ...
    if not os.path.exists(os.path.join(folder, 'output.json')):
        print(folder, 'is not a complete data folder!')
        exit(0)
    for root, _, files in os.walk(folder):
        for file in files:
            if file.find('.vtu') != -1 and file.find('boundary') == -1:
...
#     def __init__(self, tri, values, fill_value=np.nan, tol=1e-6, maxiter=400):
#         interpnd.NDInterpolatorBase.__init__(self, tri.points, values, ndim=2, fill_value=fill_value)
#         self.tri = tri
#         self.grad = interpnd.estimate_gradients_2d_global(self.tri, self.values, tol=tol, maxiter=maxiter)
...
def compute_residual(problem, pos, vel, p, mesh_size, t_id, time_arr, structured=False, draw=""):
    ...
    res = fit_spatial_derivatives(x, y, vel[t_id,:,:], p[t_id,:,:], visc)
    res += fit_time_arr_derivatives(time_arr, vel)[t_id,:,:]
    ...
    return res, np.sum(res ** 2) * (h1 * h2) ** 2, error * (h1 * h2) ** 2
...
if __name__ == "__main__":
    ...
    pos, vel, p, mesh_size, time_arr, t_id = read_transient_data(folder, 0)
    dt = time_arr[1] - time_arr[0]
    tmp_file = os.path.join('./tmp', folder_name + '.tmp')
    if os.path.exists(tmp_file):
        os.remove(tmp_file)
    processes = []
    for idx in range(len(indices)):
        # print('Start '+str(idx), flush=True)
        processes.append(subprocess.Popen(['python', 'instant-residual.py', args.path, '--output', tmp_file, '--idx', str(idx)]))
        if len(processes) >= args.ntasks:
            for p in processes:
                p.wait()
            del processes
            processes = []
    ...
    if not os.path.exists(args.output):
        with open(args.output, 'w') as f:
            f.write('problem,solver,h,residual,final_residual\n')
    
    with open(args.output,'a') as f:
        f.write(','.join([problem_name, solver, repr(mesh_size.round(decimals=10)), repr(np.sqrt(total)), repr(final)])+'\n')
    
    print('total residual =',repr(np.sqrt(total)), ', final residual =', repr(final))
