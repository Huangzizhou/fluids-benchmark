...
#         vel.append(vel_)
#         p.append(p_)
#     return pos, np.array(vel), np.array(p), mesh_size
...
def dense_sample_field(pos, new_pos, field):
    field_dense = CloughTocher2DInterpolator(pos, field, tol=1e-10)
...
def compute_residual(problem, pos, vel, p, mesh_size, t_id, time_arr, structured=False, draw=""):
    ...
    if problem == "tunnel":
        ...
        mask = np.logical_and(pos_[:,0] >= 0.4, pos_[:,1] <= 0.6)
        res[mask] = 0
    elif problem == "Lshape":
        mask = np.logical_and(pos_[:,0] <= 0.25, pos_[:,1] <= 0.12)
        res[mask] = 0
...
if __name__ == "__main__":
    ...
    for idx in range(len(indices)):
        pos, vel, p, mesh_size, time_arr, t_id = read_transient_data(folder, idx)
        # if idx < len(indices) - 1:
        res, val, err = compute_residual(problem_name, pos, vel, p, mesh_size, t_id, time_arr, structured=is_structured)
        # else:
            # res, val, err = compute_residual(problem_name, pos, vel, p, mesh_size, t_id, time_arr, structured=is_structured, draw=folder_name+'.png')
        if folder.find('tgv') != -1:
            print('[%d / %d] residual = %e, error = %e' % (idx+1, len(indices), np.sqrt(val), np.sqrt(err)), flush=True)
        else:
            print('[%d / %d] residual = %e' % (idx+1, len(indices), np.sqrt(val)), flush=True)
        res_field.append(res.copy())
        dt = time_arr[1] - time_arr[0]
        total += val * dt
        total_err += err * dt
    
    # with open(args.output,'a') as f:
        # f.write(','.join([problem_name, solver, repr(mesh_size.round(decimals=10)), repr(np.sqrt(total)), repr(np.sqrt(val)), repr(np.sqrt(total_err)), repr(np.sqrt(err)) ])+'\n')
    
    res_field = np.array(res_field).reshape(-1)

    plt.figure(dpi=100)

    group_num = 100

    plt.hist(res_field, bins=group_num)
    ...
    print(','.join([problem_name, solver, repr(mesh_size.round(decimals=10))]))
    print('total residual =',repr(np.sqrt(total)))
    print('final residual =',repr(np.sqrt(val)))
    print('max point-wise residual =',repr(np.amax(res_field)))
    if folder.find('tgv') != -1:
        print('total error =',repr(np.sqrt(total_err)))
        print('final error =',repr(np.sqrt(err)))

    # draw_field(x, y, np.linalg.norm(res[t_id,:,:], axis=1).reshape(x.shape[0],y.shape[0]), path=folder_name+'.png', data_range=[0,100], title='residual')

    # # integrate residual through time_arr and space
    # dt = time_arr[1] - time_arr[0]
    # h = x[1] - x[0]

    # residual = np.sqrt(np.sum(res ** 2)) * h * h * dt
    # print(folder_name+',', residual)
